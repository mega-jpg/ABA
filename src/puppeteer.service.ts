...
export class PuppeteerService {
  ...
  /** Crop top mặc định: 8.5333333333vw (ẩn phần header của iframe). */
   ...
   *
   * Trả về:
   * - `iframe` = `<iframe id="iframeGameFullPage">` element — DÙNG ĐỂ SCREENSHOT.
  ...
  /**
   * Chờ #iframeGameFullPage + DOM bàn (.result_left / .result_right).
   * Bắt buộc sau click bàn — kể cả chi_gui_nhom_ao hoặc retry bỏ qua tin Telegram.
   */
  private async waitForBaccaratTableReady(
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      if (found) {
        ...
        try {
          ...
          if (domReady) {
            this.logger.log(
              `✅ ${tag}Bàn Baccarat sẵn sàng${attempt > 0 ? ` sau ${attempt}s` : ''}`,
            );
            return found.frame;
          }
        } catch {
      ...
      if (attempt > 0 && attempt % 10 === 0) {
        this.logger.log(
          `⏳ ${tag}Vẫn chờ bàn load... (${attempt}/${maxAttempts}s)`,
        );
      }
    ...
    throw new Error(
      'Không tìm thấy iframe iframeGameFullPage / DOM bàn chưa sẵn sàng',
    );
  }
  ...
  private async captureIframeSafely(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        // 1) Re-lookup iframe + game frame (tránh handle stale)
        const found = await this.findGameIframe(page);
        if (!found) {
  ...
  private async sendTablePhotoToSingleGroupAo(
    groupId: string,
    photoPath: string,
  ): Promise<void> {
    const aoIdx = this.getAoGroupIndex(groupId);
    const link = this.getConfigForwardLink(
    ...
    await this.sendPhotoToSingleGroupAo(
      groupId,
      photoPath,
      this.buildBaoBanPhotoCaption(),
    );
  }

  private async sendTablePhotoToSingleGroupThat(
    groupId: string,
    photoPath: string,
  ): Promise<void> {
    if (isChiGuiNhomAo()) return;
    ...
    const link = this.getConfigForwardLinkForThat(
      'link_forward_tin_nhan_bao_ban',
      thatIdx,
    );
    if (link) {
  ...
  private async sendTablePhotoToConfiguredGroups(
    ...
    if (skipThat) {
      this.logger.log(
        `♻️ Retry ca ${ca}: bỏ qua "bao_ban_that" (ảnh bàn đã gửi)`,
      );
    }
  ...
  async findBaccaratActive(page: puppeteer.Page): Promise<void> {
    try {
      ...
      // Đợi bàn load — luôn chờ iframe + DOM (kể cả chi_gui_nhom_ao / retry bỏ qua tin)
