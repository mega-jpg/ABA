...
exports.injectToolCasinoOverlay = injectToolCasinoOverlay;
exports.updateToolCasinoOverlayText = updateToolCasinoOverlayText;
exports.removeToolCasinoOverlay = removeToolCasinoOverlay;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const TOOLCASINO_ASSETS = path.join(process.cwd(), 'ToolCasino', 'assets');
function readDataUrl(fileName, mime) {
...
const FALLBACK_ROBOT_SVG = `data:image/svg+xml,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 200" width="150" height="250">
...
async function injectToolCasinoOverlay(page, options) {
    const robotSrc = readDataUrl('robot.webp', 'image/webp') ??
        readDataUrl('robot.png', 'image/png') ??
        FALLBACK_ROBOT_SVG;
    const bubbleSrc = readDataUrl('nen111.webp', 'image/webp') ??
...
async function updateToolCasinoOverlayText(page, text) {
...
async function removeToolCasinoOverlay(page) {
    await page.evaluate(() => {
        delete window.__toolcasinoSetText;
        document.getElementById('toolcasino-overlay-style')?.remove();
        document.getElementById('prediction-drag-box')?.remove();
    });
}
