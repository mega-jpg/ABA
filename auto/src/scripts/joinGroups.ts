...
import { getEnabledGroups, loadSeedingConfig } from "../services/seedingConfig";
import { getGroupJoinInfo } from "../services/groupAccess";
...
async function loadInviteFromConfig(): Promise<{
  ...
  try {
    ...
    const group =
      ...
      cfg.groups[0];
    const joinInfo = group ? await getGroupJoinInfo(group.id) : null;
    return {
...
async function joinSessionsToGroup(params: {
  sessions: SessionsManifest["sessions"];
  targetGroup?: string;
  inviteLink?: string;
  username?: string;
  groupLabel?: string;
}): Promise<{ joined: number; already: number; failed: number }> {
...
async function main(): Promise<void> {
  ...
  if (allEnabled) {
    ...
    for (const g of groups) {
      const joinInfo = await getGroupJoinInfo(g.id);
