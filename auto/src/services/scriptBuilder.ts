import {
  SeedingConfigFile,
  PresetStepConfig,
  RandomInteractionConfig,
  CloneConfig,
} from "../types/seedingConfig";
import {
  getEnabledClones,
  resolveTargetGroupId,
} from "./seedingConfig";
import { SeedingScript, SeedingStep, randomDelay } from "../types/seeding";
...
function buildPresetSteps(steps: PresetStepConfig[]): SeedingStep[] {
  return steps.map((s) => ({
    cloneId: s.cloneId,
    action: s.action,
    payload: {
...
function pickClone(
  clones: CloneConfig[],
  strategy: RandomInteractionConfig["pickClone"],
  index: number
): string {
  if (clones.length === 0) throw new Error("Không có clone enabled");
...
function buildRandomSteps(
  clones: CloneConfig[],
  randomCfg: RandomInteractionConfig
): SeedingStep[] {
  const stepCount =
    randomDelay(randomCfg.minSteps, randomCfg.maxSteps);
  ...
  for (let i = 0; i < stepCount; i++) {
    ...
    const delay = randomDelay(randomCfg.delayMinSec, randomCfg.delayMaxSec);
...
export async function buildScriptFromConfig(
  cfg: SeedingConfigFile
): Promise<SeedingScript> {
  const clones = getEnabledClones(cfg);
  const chatId = resolveTargetGroupId(cfg);
  ...
  if (cfg.mode === "preset") {
    ...
    for (const step of steps) {
      if (!enabledCloneIds.has(step.cloneId)) {
        throw new Error(
          `Preset step dùng clone "${step.cloneId}" nhưng clone chưa enabled trong config`
        );
      }
    }
  } else {
...
export async function createScriptFromConfigFile(): Promise<SeedingScript> {
